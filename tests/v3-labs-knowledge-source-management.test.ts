...
function memoryRepository(initialState: MemoryState, options?: { failCatalogEventDelete?: boolean }) {
  ...
  function operationsFor(target: MemoryState): KnowledgeMutationOperations {
    return {
  ...
  const repository: KnowledgeMutationRepository = {
    ...
    async transaction(operation) {
      ...
      const result = await operation(operationsFor(draft));
      state = draft;
      Object.assign(repository, operationsFor(state));
      return result;
    },
  };
