...
5. **Sistema de gestion externo:** shows read-only values generated by Vase: `business.vase.ar`, the tenant UUID, and the Consumer Key. Each value has an independent copy action. This flow does not accept or store external credentials.
...
- The external-management credential view obtains values through an authenticated server-side integration boundary. It exposes only the domain, tenant UUID, and Consumer Key requested by the product design.
...
- Missing external credentials display an unavailable state and do not invent fallback values.
