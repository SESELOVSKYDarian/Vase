...
export async function DELETE(_req: NextRequest, { params }: { params: { deviceId: string } }) {
  try {
    const session = await auth()
    if (!session?.user?.companyId) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })
    const device = await WarehouseDeviceService.deleteDevice(session.user.companyId, params.deviceId)
    if (!device) return NextResponse.json({ error: 'Dispositivo no encontrado' }, { status: 404 })
    return NextResponse.json({ ok: true, ...device })
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Error al eliminar dispositivo' }, { status: 400 })
  }
}
