  ...
  return (
    <main className="product-content">
      ...
      <section className="ui-card"><h2>Grupos de sucursales</h2>
        ...
        <div className="branch-list">{groups.map((group) => <article key={group.id}>
          <code>{group.code}</code><strong>{group.name}</strong>
          <span>{group.members.map((member) =>
            branches.find((branch) => branch.id === member.branchId)?.name).filter(Boolean).join(" · ") || "Sin miembros"}</span>
        </article>)}</div>
      </section>
