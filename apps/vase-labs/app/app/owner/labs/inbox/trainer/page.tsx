...
export default async function TrainerInboxPage() {
  ...
  try {
    ...
    return <div className="space-y-6"><LabsPageHeader eyebrow="Inbox separado" title="Entrenador personal" description="Audios, instrucciones y propuestas del entrenador. Nunca se mezclan con clientes ni leads." />
      <LabsSection title="Audios" description="Los audios se transcriben con OpenAI y después generan una propuesta."><div className="space-y-2">{jobs.length ? jobs.map((job) => <article key={job.id} className="rounded-lg border border-[var(--border)] p-4"><div className="flex justify-between gap-3"><strong>Audio del entrenador</strong><LabsStatusPill label={job.status} tone={job.status === "COMPLETED" ? "success" : job.status === "FAILED" ? "danger" : "warning"} /></div><p className="mt-2 text-sm text-[var(--muted)]">{job.transcript ?? job.error ?? "Esperando transcripción…"}</p></article>) : <p className="text-sm text-[var(--muted)]">Todavía no hay audios.</p>}</div></LabsSection>
